...
\begin{frame}[fragile=singleslide]{Variáveis aleatórias discretas}
  A \textbf{função de probabilidade} (fp) atribui probabilidades a cada
  um dos possíveis valores assumidos pela variável
  \begin{equation*}
    P[X = x_i] = p(x_i), \quad i = 1, 2, \ldots
  \end{equation*}
...
\begin{frame}[fragile]{Variância}
  ...
  \begin{equation*}
    \Var(X) = \sum_{i=1}^{\infty} [x_i - \E(X)]^2 \cdot P[X=x_i]
  \end{equation*}
  ...
  onde
  \begin{equation*}
    \E(X^2) = \sum_{i = 1}^{\infty} x_{i}^{2} \cdot P[X = x_i]
  \end{equation*}
\end{frame}
...
\begin{frame}[fragile]{Esperança e variância}
  ...
  \vspace{2em}
  A esperança e a variância possuem as seguintes propriedades: \\~\\
  \begin{equation*}
    \E(aX + b) = a\E(X) + b
  \end{equation*}
  \vspace{1em}
...
\begin{frame}[fragile]{Exercícios}
  Exercícios 1--6, 9, 11 do capítulo 8 do livro (pgs. 184--187).
  \vspace{1em}
  \begin{itemize}
  \item[] Pinto, SS; Silva, CS. \textbf{Estatística, Vol I}. Rio Grande:
    Editora da FURG, 2010. [Cap. 8]
  \end{itemize}
\end{frame}
