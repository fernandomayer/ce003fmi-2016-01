...
%% <<echo=FALSE, out.width=".8\\textwidth", pdfcrop=TRUE>>=
%% quant <- c(15,21,28,25,30,11,17,12,25,20,16,23,12,10)
%% boxplot(quant, range = 0, horizontal = TRUE,
%%         ylim = c(0,40))
%% @
%% \end{frame}
...
%% \begin{frame}{Quartis}
%%   \textbf{Gráfico de caixa ou resumo dos cinco números}\\~\\
%% <<echo=FALSE, out.width=".9\\textwidth", pdfcrop=TRUE>>=
%% set.seed(123)
%% x <- rnorm(1000, 50, 4)
%% df <- data.frame(X = c(rnorm(1000, 100, 8), rnorm(1000, 50, 8)),
%%                  C = c(rep("A", 1000), rep("B", 1000)))
%% par(mfrow=c(1,2))
%% boxplot(x, range = 0)
%% boxplot(X~C, data = df, range=0)
%% @
%% \end{frame}
...
%% \begin{frame}{Quartis}
%% <<quartis1, echo=FALSE, pdfcrop=TRUE>>=
%% hom <- c(5,2,7,9,3,4,3,1,3,8)
%% mul <- c(3,5,7,4,5,6,7,6,5,4)
%% boxplot(c(hom,mul), range = 0, horizontal = TRUE,
%%         xlab = "Tempo de espera (minutos)", ylim = c(0, 10))
%% @
%% \end{frame}

%% \begin{frame}{Quartis}
%% <<quartis2, echo=FALSE, pdfcrop=TRUE>>=
%% boxplot(data.frame(hom,mul), range = 0,
